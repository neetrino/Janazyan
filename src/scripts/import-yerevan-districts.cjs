...
async function geocode(queryText, cache) {
  ...
  let lng = 44.4991;
  try {
    const query = encodeURIComponent(`${queryText}, Yerevan, Armenia`);
...
async function main() {
  ...
  try {
    ...
    for (let areaIndex = 0; areaIndex < sheets.length; areaIndex += 1) {
      ...
      const coords = await geocode(districtEn, coordsCache);
