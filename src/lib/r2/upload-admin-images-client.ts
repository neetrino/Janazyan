...
import { R2_IMAGE_FOLDERS, type R2ImageFolder } from '@/lib/r2/r2-image-folders';
...
/**
 * Compresses image files and uploads them to R2 via the admin upload API.
 */
export async function uploadAdminImagesToR2(
  files: File[],
  folder: R2ImageFolder = R2_IMAGE_FOLDERS.products,
  ...
  if (!Array.isArray(data.urls) || data.urls.length === 0) {
    throw new ApiError(
      'Upload response did not include image URLs',
      response.status,
      response.statusText,
    );
  }
