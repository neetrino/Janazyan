...
import type { DashboardTrendPoint } from '../utils/dashboard-periods';
...
export function DashboardTrendSvg({ points, chartAria }: DashboardTrendSvgProps) {
  const width = 720;
  const height = 220;
  ...
  const revenuePoints = points.map((point, index) => {
    ...
    const y =
      padding.top + plotHeight - (point.revenueAmount / maxRevenue) * plotHeight;
    return { x, y, point };
  });
    ...
    >
      <defs>
        <linearGradient id="dashboardRevenueFill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={DASHBOARD_REVENUE_COLOR} stopOpacity="0.28" />
          <stop offset="100%" stopColor={DASHBOARD_REVENUE_COLOR} stopOpacity="0.02" />
        </linearGradient>
      </defs>
